...
const getFollowers = (userId) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        const result = yield session.run('MATCH (follower:User)-[:FOLLOWS]->(following:User {user_id: $userId}) RETURN follower', { userId });
        return result.records.map((record) => record.get('follower').properties.user_id.low);
    }
    catch (error) {
